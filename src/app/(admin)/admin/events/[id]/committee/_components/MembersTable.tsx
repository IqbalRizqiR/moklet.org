...
import { createEventRoleAction } from "@/actions/eventCommittee";
import { toast } from "sonner";
import { FaTrash, FaUserPlus, FaShieldAlt, FaPlus, FaCheck, FaStar } from "react-icons/fa";
import EventStructurePreview from "./EventStructurePreview";
...
export default function MembersTable({
  ...
  const handleAddMember = async () => {
    ...
    setIsLoading(true);
    const result = await assignToOrg(selectedUserId, eventId, selectedRoleId);
    setIsLoading(false);
    if (result.error) {
  ...
  const handleRegisterMember = async () => {
    ...
    setIsLoading(true);
    const result = await registerAndAssign(regName, regEmail, eventId, regRoleId);
    setIsLoading(false);
    if (result.error) {
      toast.error(result.message);
    } else {
  ...
  const handleCreateRoleInline = async () => {
    ...
    const result = await createEventRoleAction(eventId, newRoleName, newRoleIsLeader, hierarchyLevel, newRoleLevelId || null);
    setIsLoading(false);
    if (result.error) {
      toast.error(result.message);
    } else {
  ...
  const handleRemoveMember = async (userId: string, name: string) => {
    ...
    setIsLoading(true);
    const result = await removeFromOrg(userId, eventId);
    setIsLoading(false);
  ...
  const handleRoleChange = async (userId: string, roleId: string) => {
    setIsLoading(true);
    const result = await updateOrgRole(userId, eventId, roleId);
    setIsLoading(false);
    if (result.error) toast.error(result.message);
    else {
      toast.success(result.message);
  ...
  const handleAssignTemplate = async (userId: string, templateId: string) => {
    setIsLoading(true);
    const result = await assignTemplateAction(userId, eventId, templateId);
    setIsLoading(false);
    if (result.error) toast.error(result.message);
    else {
      toast.success(result.message);
  ...
  const handleTogglePermission = async (userId: string, permission: string, has: boolean) => {
    setIsLoading(true);
    const result = has
      ? await revokePermissionAction(userId, eventId, permission)
      : await grantPermissionAction(userId, eventId, permission);
    setIsLoading(false);
    if (result.error) toast.error(result.message);
    else {
      toast.success(result.message);
            ...
            <div className="p-6 space-y-4 overflow-y-auto flex-1">
              ...
              {pendingMember && (
                <div className="border-t border-gray-200/30 pt-3">
                  ...
                  <EventStructurePreview members={previewMembers} pendingMember={pendingMember} levels={levels} />
                </div>
              )}
            </div>
