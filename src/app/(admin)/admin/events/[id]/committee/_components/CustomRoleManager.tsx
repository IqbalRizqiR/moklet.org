...
import { createEventRoleAction, updateEventRoleAction, deleteEventRoleAction } from "@/actions/eventCommittee";
...
export default function CustomRoleManager({
  ...
  const handleCreateLevel = async () => {
    ...
    const result = await createLevel(eventId, newLevelName.trim(), maxOrder);
    setIsLoading(false);
  ...
  const moveLevel = async (index: number, direction: "up" | "down") => {
    ...
    setLevels(reordered);
    const result = await reorderLevels(eventId, reordered.map((l) => ({ id: l.id, order: l.order })));
    if (result.error) toast.error(result.message);
  };
  ...
  const handleCreateRole = async () => {
    ...
    const result = await createEventRoleAction(eventId, newRoleName.trim(), newRoleIsLeader, hierarchyLevel, newRoleLevelId || null);
    setIsLoading(false);
    if (result.error) toast.error(result.message);
    else {
      toast.success(result.message);
  ...
  const handleUpdateRole = async (id: string) => {
    ...
    const hierarchyLevel = level ? level.order : 5;
    const result = await updateEventRoleAction(id, eventId, editRoleName.trim(), editRoleIsLeader, hierarchyLevel, editRoleLevelId || null);
    setIsLoading(false);
    if (result.error) {
  ...
  const handleDeleteRole = async (id: string) => {
    ...
    setIsLoading(true);
    const result = await deleteEventRoleAction(id, eventId);
    setIsLoading(false);
    if (result.error) toast.error(result.message);
    else {
      toast.success(result.message);
